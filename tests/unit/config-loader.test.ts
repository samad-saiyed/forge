...
describe("Configuration File Loader", () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), "forge-config-test-"));
  });
  ...
  it("discovers and loads forge.config.ts", async () => {
    const configPath = join(tempDir, "forge.config.ts");
    writeFileSync(
      configPath,
  ...
  it("discovers and loads forge.config.js", async () => {
    const configPath = join(tempDir, "forge.config.js");
  ...
  it("discovers and loads forge.config.mjs", async () => {
    const configPath = join(tempDir, "forge.config.mjs");
  ...
  it("throws an error when multiple config files create ambiguity", async () => {
    writeFileSync(join(tempDir, "forge.config.ts"), `export default { server: { port: 1111 } };`);
    writeFileSync(join(tempDir, "forge.config.js"), `export default { server: { port: 2222 } };`);
  ...
  it("throws an error when config module fails to load", async () => {
    writeFileSync(join(tempDir, "forge.config.js"), `this is invalid javascript syntax !@#$`);
